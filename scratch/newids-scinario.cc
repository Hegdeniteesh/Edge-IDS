...
void DummyPacketApp::Setup(Ptr<Socket> socket, Address address, uint32_t packetSize, uint32_t nPackets, DataRate dataRate) {
    ...
    NS_LOG_INFO("=== DummyPacketApp Setup ===");
    NS_LOG_INFO("  Socket: " << m_socket);
    if (InetSocketAddress::IsMatchingType(m_peer)) {
        InetSocketAddress isa = InetSocketAddress::ConvertFrom(m_peer);
        NS_LOG_INFO("  Destination: " << isa.GetIpv4() << ":" << isa.GetPort());
    } else if (Inet6SocketAddress::IsMatchingType(m_peer)) {
        Inet6SocketAddress isa6 = Inet6SocketAddress::ConvertFrom(m_peer);
        NS_LOG_INFO("  Destination (IPv6): " << isa6.GetIpv6() << ":" << isa6.GetPort());
    } else {
        NS_LOG_INFO("  Destination: Unknown address type");
    }
    NS_LOG_INFO("  Packet Size: " << m_packetSize << " bytes");
    NS_LOG_INFO("  Number of Packets: " << m_nPackets);
    NS_LOG_INFO("  Data Rate: " << m_dataRate);
    NS_LOG_INFO("  Bitrate: " << m_dataRate.GetBitRate() << " bps");
    NS_LOG_INFO("  Inter-packet interval: " << (m_packetSize * 8.0 / m_dataRate.GetBitRate()) << " seconds");
    NS_LOG_INFO("============================");
}
...
void DummyPacketApp::StartApplication(void) {
    ...
    NS_LOG_INFO("=== DummyPacketApp Starting ===");

    if (Inet6SocketAddress::IsMatchingType(m_peer)) {
        m_socket->Bind6();
        NS_LOG_INFO("  Binding to IPv6 socket");
    } else {
        m_socket->Bind();
        NS_LOG_INFO("  Binding to IPv4 socket");
    }

    // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    // LOG THE SOURCE IP:PORT AFTER BINDING (ADDED AS REQUESTED)
    Address localAddr;
    if (m_socket->GetSockName(localAddr) == 0) {
        if (InetSocketAddress::IsMatchingType(localAddr)) {
            InetSocketAddress localIsa = InetSocketAddress::ConvertFrom(localAddr);
            NS_LOG_INFO("  Socket bound to SOURCE: " << localIsa.GetIpv4() << ":" << localIsa.GetPort());
        } else if (Inet6SocketAddress::IsMatchingType(localAddr)) {
            Inet6SocketAddress localIsa6 = Inet6SocketAddress::ConvertFrom(localAddr);
            NS_LOG_INFO("  Socket bound to SOURCE (IPv6): " << localIsa6.GetIpv6() << ":" << localIsa6.GetPort());
        } else {
            NS_LOG_WARN("  Could not determine source address type");
        }
    } else {
        NS_LOG_WARN("  Failed to retrieve socket source address (GetSockName failed)");
    }
    // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

    int connectResult = m_socket->Connect(m_peer);
    if (connectResult == 0) {
        NS_LOG_INFO("  Successfully connected to peer");
    } else {
        NS_LOG_WARN("  Failed to connect to peer (error code: " << connectResult << ")");
    }

    m_socket->SetRecvCallback(MakeCallback(&DummyPacketApp::HandleRead, this));
    NS_LOG_INFO("  Receive callback registered");
...
void DummyPacketApp::StopApplication(void) {
    ...
    if (m_sendEvent.IsPending()) {
        ...
        NS_LOG_INFO("  Cancelled pending send event");
    }
    if (m_socket) {
        m_socket->Close();
        NS_LOG_INFO("  Socket closed");
    }
    NS_LOG_INFO("=== DummyPacketApp Stopped ===");
}
...
void DummyPacketApp::SendPacket(void) {
    ...
    if (actual > 0) {
        m_packetsSent++;
        NS_LOG_INFO("Dummy packet SENT: Size=" << packet->GetSize() << " bytes, Sent=" << actual << " bytes, Total Sent=" << m_packetsSent);
    } else {
        NS_LOG_WARN("Dummy packet SEND FAILED. Return code: " << actual);
    }
    if (m_packetsSent < m_nPackets) {
        ScheduleTx();
    } else {
        NS_LOG_INFO("All dummy packets sent. Waiting for ACK or timeout.");
    }
}

void DummyPacketApp::ScheduleTx(void) {
    if (m_running) {
        double bitTime = m_packetSize * 8.0 / static_cast<double>(m_dataRate.GetBitRate());
        Time tNext(Seconds(bitTime));
        m_sendEvent = Simulator::Schedule(tNext, &DummyPacketApp::SendPacket, this);
        NS_LOG_INFO("Scheduled next dummy packet in " << tNext.GetSeconds() << " seconds");
    }
}
    ...
    while ((packet = socket->RecvFrom(from))) {
        if (packet->GetSize() > 0) {
            if (InetSocketAddress::IsMatchingType(from)) {
                ...
                NS_LOG_INFO("Dummy packet ACK RECEIVED from " << srcIp << ", Packet Size: " << packet->GetSize() << " bytes");
            } else {
                NS_LOG_INFO("Dummy packet ACK RECEIVED from unknown address type, Packet Size: " << packet->GetSize() << " bytes");
            }
            // Verification successful
            if (!m_verificationCallback.IsNull()) {
                m_verificationCallback(true);
                NS_LOG_INFO("Verification callback invoked with SUCCESS");
            }
            StopApplication();
            break;
        } else {
            NS_LOG_WARN("Received dummy ACK packet with zero size");
        }
    }
}
...
void SendDummyPacketForVerification(Ptr<Node> sourceNode, Ipv4Address destIp, uint16_t destPort, Callback<void, bool> verificationCallback) {
    NS_LOG_INFO("=== Initiating Dummy Packet Verification ===");
    NS_LOG_INFO("Source Node: " << sourceNode->GetId() << " -> Destination: " << destIp << ":" << destPort);

    // Create a socket
    TypeId tid = TypeId::LookupByName("ns3::UdpSocketFactory");
    Ptr<Socket> sinkSocket = Socket::CreateSocket(sourceNode, tid);
    if (!sinkSocket) {
        NS_LOG_ERROR("Failed to create UDP socket for dummy verification");
    ...
    app->SetStartTime(Seconds(now));
    app->SetStopTime(Seconds(now + 2.0)); // Timeout after 2 seconds

    NS_LOG_INFO("DummyPacketApp scheduled to start at " << now << "s and timeout at " << (now + 2.0) << "s");
}
